...
public class MagazineRestController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<MagazineEntity> replaceMagazine(@RequestBody MagazineEntity newMag, @PathVariable Long id) {
        return magazineRepository.findById(id)
                .map(mag -> {
                    mag.setTitle(newMag.getTitle());
                    mag.setPrice(newMag.getPrice()); // This updates the price!
